...
const App = ()=> {
  ...
  return (
    <Container className="px-0" style={{maxWidth:'500px'}}>
      <Header/>
      <Routes>
        <Route exact path="/" element={<ContactList contacts={searchTerm.length<1?contacts:searchRes} deleteContactHandler={removeContactHandler} editContactHandler={editContactHandler} term={searchTerm} searchKeyword={searchHandler} />}/> 
        <Route path="/add" element={<AddContact addContactHandler={addContactHandler} />}/> 
        <Route path="/edit/:idData" element={<EditContact  updateContactHandler={updateContactHandler}/>}/> 
      </Routes>
    </Container>
  );
}
